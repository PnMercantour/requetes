...
CREATE SERVER srv_geonature FOREIGN DATA WRAPPER postgres_fdw OPTIONS (service 'geonature');
...
FROM SERVER srv_geonature INTO gn_synthese;
...
-- REMPLACE par la vue matérialisée synthese_avec_partenaires (même format, plus d'attributs) qui est plus performante.
CREATE MATERIALIZED VIEW gn_synthese.synthese_proxy AS
SELECT synthese.id_synthese,
    synthese.cd_nom,
    synthese.date_min,
    synthese.centroid_2154
FROM gn_synthese.synthese WITH DATA;
...
SELECT unp.*,
    count(id_synthese)
FROM ag_pasto.c_unite_pastorale_unp unp
    LEFT JOIN gn_synthese.synthese_proxy ON (st_within (centroid_2154, geom))
GROUP BY unp.id;
create index on gn_synthese.synthese_avec_partenaires (cd_nom);
create index on gn_synthese.synthese_avec_partenaires (date_min);